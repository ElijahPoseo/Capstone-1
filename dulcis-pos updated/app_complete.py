...
# ==================== ENCRYPTION ENGINE ====================
# Keys are stored in a separate "keys" subfolder, excluded from any code zip/backup.
...
_mail_sender_name = os.environ.get('MAIL_DEFAULT_SENDER_NAME', 'StockSecure POS')
app.config['MAIL_DEFAULT_SENDER'] = (_mail_sender_name, os.environ.get('MAIL_USERNAME', ''))
mail = Mail(app)

def _reset_serializer():
    return URLSafeTimedSerializer(app.config['SECRET_KEY'], salt='password-reset')

# ==================== OTP STORE (database-backed, restart-safe) ====================
...
def _generate_otp(email, user_id):
    """Generate a 6-digit OTP, persist it to the DB, and return the code."""
    code = f"{random.randint(0, 999999):06d}"
    expires = (datetime.now(timezone.utc) + timedelta(minutes=10)).isoformat()
    _ensure_otp_table()
    with sqlite3.connect(DB_PATH) as conn:
        conn.execute(
            "INSERT OR REPLACE INTO otp_store (email, code, user_id, expires_at, attempts) VALUES (?, ?, ?, ?, 0)",
            (email, code, user_id, expires)
        )
        conn.commit()
    return code

def _verify_otp(email, code):
    """Verify OTP from DB. Returns user_id on success, None on failure."""
    _ensure_otp_table()
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if row['code'] != code.strip():
            conn.execute("UPDATE otp_store SET attempts = ? WHERE email = ?", (new_attempts, email))
            conn.commit()
            return None
        conn.execute("DELETE FROM otp_store WHERE email = ?", (email,))
        conn.commit()
...
def migrate_must_change_password():
    """Add must_change_password column to existing databases that predate this fix."""
    with sqlite3.connect(DB_PATH) as conn:
        try:
            conn.execute("ALTER TABLE users ADD COLUMN must_change_password INTEGER DEFAULT 0")
            conn.commit()
            print("[MIGRATION] Added must_change_password column to users table.")
        except Exception:
...
def admin_ledger():
    ...
    if integrity_result['status'] == 'key_mismatch':
        ...
        integrity_result = ledger.verify_integrity(max_entries=verify_limit)
        ...
        with get_db() as conn:
            ...
            for item_data in sale_items_data:
                ...
                if result.rowcount == 0:
                    raise ValueError(f"Insufficient stock for {item_data['name']} (concurrent sale detected)")
...
if __name__ == "__main__":
    ...
    # debug=False — never expose the debugger in a deployed system
