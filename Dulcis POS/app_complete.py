from flask import Flask, render_template, request, redirect, session, url_for, flash, jsonify, send_file, Response
...
EXTERNAL_BACKUP_DIR = os.path.join(os.path.dirname(BASE_DIR), "Dulcis_Backups")
MAX_LOCAL_BACKUPS = 30    # keep up to 30 local backups
MAX_EXTERNAL_BACKUPS = 90  # keep up to 90 external backups (redundant off-folder copy)
BACKUP_HOUR = 23   # run the daily backup at 11:00 PM (end of business day)
BACKUP_MINUTE = 59
...
def get_next_or_number(conn, series='OR'):
    """
    Atomically assign the next sequential Official Receipt number.

    Must be called with an already-open `conn` from get_db(), inside the
    same transaction as the sales INSERT, so the number and the sale are
    committed (or rolled back) together. SQLite serializes writers, so the
    UPDATE + SELECT pair below cannot race with another checkout — the
    second writer simply blocks until the first transaction commits.

    The series is strictly sequential with no gaps and no reused numbers,
    even for later-voided/cancelled sales — so this function should only
    ever be called once per sale, and the assigned number should never be
    re-issued.
    """
    conn.execute(
        "UPDATE or_sequence SET next_number = next_number + 1, "
        "updated_at = CURRENT_TIMESTAMP WHERE series_name = ?",
        (series,)
    )
    row = conn.execute(
        "SELECT next_number, prefix, pad_width FROM or_sequence WHERE series_name = ?",
        (series,)
    ).fetchone()
    if row is None:
        raise RuntimeError(f"OR sequence '{series}' is not initialized")

    assigned_number = row['next_number'] - 1
    or_number = f"{row['prefix']}{str(assigned_number).zfill(row['pad_width'])}"
    return or_number
...
# ==================== BACKUP FUNCTIONS ====================
def create_backup(silent=True, label="auto"):
    """Create backup — saves inside project/backups AND to an external
    Dulcis_Backups folder one level above the project, so a single lost
    or corrupted drive doesn't take out every copy."""
    if not os.path.exists(DB_PATH):
        return False, "Database not found"
    try:
        timestamp = get_utc_now().strftime('%Y%m%d_%H%M%S')
        backup_name = f"dulcis_backup_{label}_{timestamp}.db"

        # ── 1. Local backup (inside project) ──────────────────────────
        local_path = os.path.join(BACKUP_DIR, backup_name)
        shutil.copy2(DB_PATH, local_path)
        local_backups = sorted(
            [f for f in os.listdir(BACKUP_DIR) if f.startswith('dulcis_backup_')],
            reverse=True
        )
        while len(local_backups) > MAX_LOCAL_BACKUPS:
            os.remove(os.path.join(BACKUP_DIR, local_backups.pop()))

        # ── 2. External backup (one level above project) ──────────────
        try:
            os.makedirs(EXTERNAL_BACKUP_DIR, exist_ok=True)
            ext_path = os.path.join(EXTERNAL_BACKUP_DIR, backup_name)
            shutil.copy2(DB_PATH, ext_path)
            ext_backups = sorted(
                [f for f in os.listdir(EXTERNAL_BACKUP_DIR) if f.startswith('dulcis_backup_')],
                reverse=True
            )
            while len(ext_backups) > MAX_EXTERNAL_BACKUPS:
                os.remove(os.path.join(EXTERNAL_BACKUP_DIR, ext_backups.pop()))
            ext_ok = True
        except Exception as ext_e:
            ext_ok = False
            if not silent:
                print(f"External backup warning: {ext_e}")
...
def auto_backup_loop():
    """Background thread - automatic backup once per day at end-of-business
    (BACKUP_HOUR:BACKUP_MINUTE), mirrored to both local and external folders."""
    while True:
        now = datetime.now()
        ...
        create_backup(silent=True, label="auto")
...
def init_db():
    ...
    with get_db() as conn:
        ...
        """)
        # Sequential numbering for Official Receipts. Numbers are assigned
        # atomically and are NEVER reused, even for voided/cancelled sales,
        # so the printed OR series stays unbroken.
        conn.execute("""
            CREATE TABLE IF NOT EXISTS or_sequence (
                series_name TEXT PRIMARY KEY,
                next_number INTEGER NOT NULL DEFAULT 1,
                prefix TEXT NOT NULL DEFAULT '',
                pad_width INTEGER NOT NULL DEFAULT 9,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """)
        conn.execute("""
            INSERT OR IGNORE INTO or_sequence (series_name, next_number, prefix, pad_width)
            VALUES ('OR', 1, '', 9)
        """)
        conn.execute("""
        ...
        try:
            conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_sales_or_number ON sales(or_number)")
            conn.commit()
        except Exception:
            pass
        ...
        # Migration: add expiration columns if they do not exist (for existing databases)
        existing_cols = [row[1] for row in conn.execute("PRAGMA table_info(products)").fetchall()]
        if "expiration_date" not in existing_cols:
            try:
                conn.execute("ALTER TABLE products ADD COLUMN expiration_date DATE")
                conn.commit()
            except Exception:
                pass
        if "expiry_warning_days" not in existing_cols:
            try:
                conn.execute("ALTER TABLE products ADD COLUMN expiry_warning_days INTEGER DEFAULT 30")
                conn.commit()
            except Exception:
                pass
...
def login():
    ...
    if request.method == "POST":
        ...
        return render_template("login.html", failed_attempts=session.get('login_failed_attempts', 0))
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        if not username or not password:
            flash("Username and password are required.", "danger")
            return render_template("admin/login.html", failed_attempts=failed_attempts)
        ...
        return render_template("admin/login.html", failed_attempts=session.get('admin_login_failed_attempts', 0))
        ...
        stats = {
            ...
            'expiring_soon': conn.execute("""
                ...
                WHERE is_active = 1 AND expiration_date IS NOT NULL
                  AND julianday(expiration_date) - julianday('now') <= expiry_warning_days
            """).fetchone()[0],
...
def _build_sales_csv(date_from, date_to):
    """Build a CSV export of sales for the given date range (includes the
    sequential OR number). Matches the same filter used by the admin/manager
    Sales History pages (completed, non-cancelled sales)."""
    with get_db() as conn:
        rows = conn.execute("""
            SELECT s.*, u.username as cashier_name,
    ...
    writer.writerow([
        'OR Number', 'Transaction ID', 'Date & Time (UTC)', 'Customer', 'Cashier',
        'Items', 'Payment Method', 'VATable Sales', 'VAT (12%)',
    ...
    for r in rows:
        ...
        tax = d.get('tax_amount') or 0
        vatable = round(total - tax, 2)
        writer.writerow([
            d.get('or_number') or '',
            d.get('transaction_id') or '',
            d.get('created_at') or '',
            d.get('customer_name') or 'Walk-in Customer',
            d.get('cashier_name') or '',
            d.get('item_count') or 0,
            (d.get('payment_method') or '').title(),
            f"{vatable:.2f}",
            f"{tax:.2f}",
            f"{(d.get('discount_amount') or 0):.2f}",
            f"{total:.2f}",
            (d.get('payment_status') or '').title(),
        ])

    filename = f"sales_{date_from}_to_{date_to}.csv"
    return Response(
        output.getvalue(),
        mimetype='text/csv',
...
@app.route("/manager/sales/export")
@login_required
@manager_required
def manager_sales_export():
    date_from = request.args.get('from', get_utc_now().strftime('%Y-%m-%d'))
    date_to = request.args.get('to', get_utc_now().strftime('%Y-%m-%d'))
    log_audit("SALES_EXPORTED", "sales", None, None, {'from': date_from, 'to': date_to, 'by': session.get('username')})
    return _build_sales_csv(date_from, date_to)
...
def add_product():
    ...
    expiry_warning_days = int(request.form.get('expiry_warning_days', 30) or 30)
        ...
        try:
            cursor = conn.execute("""
                INSERT INTO products (name, description, price, stock, category_id, 
                                    min_stock_level, barcode, image, expiration_date, expiry_warning_days)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (name, description, price, stock, category_id, min_stock, encrypted_barcode, image_filename, expiration_date, expiry_warning_days))
            conn.commit()
    ...
    with get_db() as conn:
        ...
        conn.execute("""
            ...
            SET name = ?, description = ?, price = ?, stock = ?, category_id = ?,
                min_stock_level = ?, barcode = ?, image = ?, is_active = ?,
                expiration_date = ?, expiry_warning_days = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        """, (name, description, price, stock, category_id, min_stock, encrypted_barcode,
              image_filename, is_active, expiration_date, expiry_warning_days, product_id))
...
@app.route("/admin/product/reactivate/<int:product_id>", methods=["POST"])
@login_required
@admin_required
def reactivate_product(product_id):
    reactivated_name = None
    with get_db() as conn:
        product = conn.execute("SELECT * FROM products WHERE id = ?", (product_id,)).fetchone()
        if product:
            conn.execute("UPDATE products SET is_active = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?", (product_id,))
            conn.commit()
            reactivated_name = product['name']
            flash("Product reactivated successfully!", "success")
        else:
            flash("Product not found.", "danger")

    if reactivated_name:
        ledger.log("PRODUCT_REACTIVATE", session['user_id'], {
            'product_id': product_id, 'name': reactivated_name
        })
        log_audit("PRODUCT_REACTIVATED", "products", product_id, None, {'name': reactivated_name})
...
def admin_reset_user_password(user_id):
    ...
    with get_db() as conn:
        ...
        conn.commit()
        flash(f"Password for '{target_username}' has been reset.", "success")

    # Audit trail records that a reset happened, never the new password itself
    ledger.log("USER_PASSWORD_RESET", session['user_id'], {
        'target_user_id': user_id,
        'target_username': target_username,
        'reset_by': session.get('username')
    })
    log_audit("USER_PASSWORD_RESET", "users", user_id, None, {
        'target_username': target_username
    })
...
def create_sale():
    ...
    discount_type = data.get('discount_type', 'none')   # 'none' | 'sc' | 'pwd'
    discount_id = data.get('discount_id', '') or ''      # OSCA/PWD card number

    if discount_type not in ['none', 'sc', 'pwd']:
        discount_type = 'none'
    ...
    try:
        with get_db() as conn:
            ...
            # SC/PWD discount (RA 9994 / RA 10754): 20% off VAT-exclusive
            # price + VAT exemption, when applicable.
            final_discount = 0.0
            if discount_type in ('sc', 'pwd'):
                vat_exclusive = total_amount / 1.12
                final_discount = round(vat_exclusive * 0.20, 2)
                total_amount = round(vat_exclusive - final_discount, 2)
                tax_amount = 0.0  # VAT-exempt

            transaction_id = f"TXN-{get_utc_now().strftime('%Y%m%d%H%M%S')}-{secrets.token_hex(4).upper()}"
            # Sequential Official Receipt number — strictly increasing,
            # never reused, assigned in the same DB transaction as the
            # sale so it can never be skipped or duplicated.
            or_number = get_next_or_number(conn)

            cursor = conn.execute("""
                INSERT INTO sales (transaction_id, or_number, user_id, total_amount, tax_amount,
                                 discount_amount, payment_method, payment_status,
                                 digital_reference, digital_amount_paid, customer_name)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (transaction_id, or_number, session['user_id'], total_amount, tax_amount,
                  final_discount,
...
def get_product(product_id):
    ...
    return jsonify({
        ...
        'expiry_warning_days': product['expiry_warning_days'] or 30
    })
...
def get_expiring_products():
    with get_db() as conn:
        ...
        rows = conn.execute("""
            SELECT p.id, p.name, p.stock, p.expiration_date, p.expiry_warning_days,
                   c.name as category_name,
                   julianday(p.expiration_date) - julianday(?) as days_until_expiry
            FROM products p
            LEFT JOIN categories c ON p.category_id = c.id
            WHERE p.is_active = 1 AND p.expiration_date IS NOT NULL
              AND julianday(p.expiration_date) - julianday(?) <= p.expiry_warning_days
    ...
    for r in rows:
        ...
        d['status'] = 'expired' if days < 0 else ('critical' if days <= 7 else 'warning')
        result.append(d)
    return jsonify(result)
...
@admin_required
def api_backup_now():
    """Manual backup triggered by an admin button."""
    success, result = create_backup(silent=False, label="manual")
    if success:
        log_audit("MANUAL_BACKUP", table_name="system", new_values={"backup_file": result})
        return jsonify({"success": True, "message": f"Backup saved: {result}", "filename": result})
...
def _escpos_receipt(sale_data, cfg):
    ...
    buf += BOLD_ON
    buf += enc(f"OR No. : {sale_data.get('or_number','')}") + LF
    buf += BOLD_OFF
    ...
    disc_type   = sale_data.get('discount_type', 'none')
    disc_amount = float(sale_data.get('discount_amount', 0) or 0)
    disc_id     = sale_data.get('discount_id', '') or ''
    if disc_type in ('sc', 'pwd'):
        buf += row("VAT 12%",         "VAT-Exempt")
        buf += row("VAT-Exempt Sale", f"P{float(total):.2f}")
        disc_label = "Senior Citizen Discount (20%)" if disc_type == 'sc' else "PWD Discount (20%)"
        buf += row(disc_label,        f"-P{disc_amount:.2f}")
        if disc_id:
            buf += enc(f"  ID/Card No: {disc_id}") + LF
    else:
    ...
    buf += ALIGN_CENTER
    if disc_type in ('sc', 'pwd'):
        disc_label_short = "Senior Citizen" if disc_type == 'sc' else "PWD"
        buf += enc(f"VAT-Exempt Sale: P{float(total):.2f}") + LF
        buf += enc(f"{disc_label_short} Discount: -P{disc_amount:.2f}") + LF
    else:
    ...
    buf += LF
    buf += ALIGN_CENTER
    buf += enc(sale_data.get('or_number', '') or sale_data.get('transaction_id', '')) + LF
    buf += LF * 4
