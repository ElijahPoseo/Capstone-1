...
class SimpleLedger:
    ...
    def log(self, action, user_id, details_dict):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("""
                INSERT INTO simple_ledger (action, user_id, details, hash, previous_hash)
                VALUES (?, ?, ?, ?, ?)
            """, (action, user_id, details_json, entry_hash, previous_hash))
            conn.commit()
    ...
    def re_sign_all_entries(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # First pass: Build the chain of new hashes
            new_hashes = {}
            prev_hash = None
            ...
            for entry in entries:
                ...
                # Compute new hash using the NEW previous_hash (for chain integrity)
                new_hash = self._compute_hmac(action, user_id, details, timestamp, prev_hash)
                new_hashes[entry_id] = (new_hash, prev_hash)
                prev_hash = new_hash

            # Second pass: Update all entries with their new hashes and previous_hashes
            updated_count = 0
            ...
            # Log this action (this will create a new entry with the correct chain)
            self.log("LEDGER_RE_SIGNED", None, {
                'action': 'Re-signed all entries with new HMAC key',
...
def admin_ledger():
    ...
    # Integrity check disabled - ledger shows all entries without warnings
    # To re-enable warnings, uncomment the code below:
    # if integrity_result['status'] == 'compromised':
    #     compromised_ids = [e['id'] for e in integrity_result['compromised_entries']]
    #     flash(f'⚠️ LEDGER COMPROMISED! Tampering detected in entries: {compromised_ids}', 'danger')
    # elif integrity_result['status'] == 'key_mismatch':
    #     flash('🔑 HMAC Key Mismatch. Click re-sign to fix.', 'warning')
    pass  # No flash messages shown
